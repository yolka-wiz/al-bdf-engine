...
| P3 | Vertical mark offsets dropped | TJ spacing can't express y-offset; zero-width marks emit inline at baseline | diacritics render at baseline, not above the letter | Fix planned — `plans/P3-vertical-mark-offsets.md` (emit per-glyph `Ts` text rise); implementation pending |
